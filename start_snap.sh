...
# oracle java binary 
export JAVA_BIN_DIR=$SNAP/jre/bin

export PYTHON_PATH=$SNAP/usr/lib/python2.7/dist-packages

mkdir -p $SNAP_USER_DATA
cd $SNAP_USER_DATA

echo "running in directory :  $SNAP_USER_DATA/fw"

if [ ! -d "scripts" ]; then
  echo "'scripts' dir not found ; copying reference one ..."
  cp -r $SNAP/scripts .
fi

if [ ! -d "test_data" ]; then
  echo "'test_data' dir not found ; copying reference one ..."
  cp -r $SNAP/test_data .
fi

if [ ! -d "db" ]; then
  echo "'db' dir not found ; copying reference one ..."
  cp -r $SNAP/db .
fi

if [ ! -f "config_gui.ini" ]; then
  echo "'config_gui.ini' file not found ; copying reference one ..."
  cp $SNAP/config_gui.ini .
fi

$SNAP/jre/bin/java -jar $SNAP/dist/FittingWizard.jar
